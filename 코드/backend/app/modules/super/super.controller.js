...
superController.delInq = async (req, res) => {
  try {
    const cst_id = req.body.cst_id;
    if (!cst_id) {
      return res.status(400).send('ID가 필요합니다.');
    }
    const deleted = await superModel.deleteInq(cst_id);
    ...
    return res.status(500).json({ error: "delete err." });
  }
}

// 문의 전체 목록 조회
superController.getAllInquiries = async (req, res) => {
  try {
    const page = parseInt(req.query.page) || 1;
    const pagerow = parseInt(req.query.pagerow) || 10;

    const inquiries = await superModel.getAllInquiries(page, pagerow);
    const totalInquiries = await superModel.getTotalInquiries();

    if (!inquiries || inquiries.length === 0) {
      return res.status(200).json({ inquiries: [], total_count: 0 });
    }

    return res.status(200).json({ inquiries: inquiries, total_count: totalInquiries });
  } catch (error) {
    console.error(error);
...
// 문의 상태 토글 업데이트 (답변완료 ↔ 미답변)
superController.updateInquiryStatus = async (req, res) => {
  try {
    ...
    const updated = await superModel.updateInquiryStatus(cst_id);
    
    if (!updated) {
      return res.status(404).json({ error: "업데이트 실패" });
    }
    
    res.json({ success: true, message: '상태가 토글되었습니다.' });
  } catch (error) {
...
// 미답변(답변대기) 건수 조회
superController.getPendingInquiriesCount = async (req, res) => {
  try {
    const count = await superModel.getPendingInquiriesCount();
    return res.status(200).json({ pending_count: count });
  } catch (e) {
    console.error('Failed to get pending count:', e);
    return res.status(500).json({ pending_count: 0 });
  }
}
/** 주문*/
//주문 관리

...
superModel.deleteInq = async (cst_id) => {
...
// 문의 전체 목록 조회
superModel.getAllInquiries = async (page, pagerow) => {
  let inquiries = [];
  let query = db
    .select('wb_inquiry.*', 'M.mem_nickname', 'C.cat_name')
    .from('wb_inquiry')
    .where('cst_status', '=', 'Y')
    .leftJoin('wb_member AS M', 'wb_inquiry.mem_idx', '=', 'M.mem_idx')
    .leftJoin('wb_inquiry_category AS C', 'wb_inquiry.cat_id', '=', 'C.cat_id')
...
superModel.getTotalInquiries = async () => {
  let totalInquiries = 0;
  await db('wb_inquiry')
    .count('cst_id as total')
    .where('cst_status', 'Y')
    .then(rows => {
      totalInquiries = rows[0].total;
    })
    .catch(e => {
      console.error(e);
      totalInquiries = 0;
...
// 미답변(답변대기) 문의 건수 조회
superModel.getPendingInquiriesCount = async () => {
  try {
    let count = 0;
    await db('wb_inquiry')
      .count('cst_id as total')
      .where('cst_status', 'Y')
      .andWhere('cst_step', '답변대기')
      .then(rows => {
        count = rows[0].total;
      })
      .catch(e => {
...
superModel.updateInquiryStatus = async (cst_id) => {
  try {
    // 1. 현재 상태 조회
    ...
    if (!inquiry) {
      console.error("Inquiry not found:", cst_id);
      return false;
    }
    
    // 2. 현재 상태에 따라 반대값으로 토글
    const newStatus = inquiry.cst_step === '답변완료' ? '답변대기' : '답변완료';
    
    // 3. 업데이트 실행
    const result = await db('wb_inquiry')
      .where('cst_id', cst_id)
      .update({ cst_step: newStatus });

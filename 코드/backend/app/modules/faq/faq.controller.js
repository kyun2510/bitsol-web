...
faqController.addFaqCategory = async(req, res) => {
    try {
        upload(req, res, async (err) => {
            ...
            const mem_idx = req.body?.mem_idx ?? 0;

            const newfaqCatetory = await faqModel.addfaqCategory(fac_title, mem_idx);

            // console.log (newfaqCatetory);
            return res.status(200).json({ message: `${newfaqCatetory} 카테고리가 생성되었습니다.` });
        })
...
faqController.getFaqListById = async(req, res) => {
    try {
        const page = req.query.page || 1; // 페이지 쿼리가 없다면 기본값은 0입니다.
        const fac_idx = req.params?.facIdx;
        ...
        // 데이터베이스에서 상품 목록을 조회
        const FaqsListById  = await faqModel.getFaqListById(fac_idx, page);

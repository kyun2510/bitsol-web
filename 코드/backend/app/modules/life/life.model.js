...
// 공지 작성하기(쓰기)v
lifeModel.addPost = async (title,subtitle,content) => {
    const db = database();
...
//공지글 수정하기 (PUT)v
lifeModel.updatePost = async (updateData) => {
    const db = database();    
    let result = null;
    ...
    return await lifeModel.getPostById(updateData.not_id);
};
// 공지글들 가져오기
lifeModel.getPosts = async (page=0) => {
    const db = database();
    let post = [];
    let query = db
        .select('N.*')
...
// 공지글 개별 조회
lifeModel.getPostById = async (not_id) => {
    const db = database();
    let post = null;
...
//게시글 지우기v
lifeModel.deletePost = async (not_idx) => {
    try {
